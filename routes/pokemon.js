...
pokemon.post("/", async(req, res, next) => {
    ...
    if(pok_name && pok_height && pok_weight && pok_base_experience){
        ...
        if(rows.affectedRows == 1){
            return res.status(200).json({code: 201, message: "Pokémon insertado correctamente"});
        }
...
pokemon.put("/:id([0-9]{1,3})", async(req, res, next) => {
    ...
    if(pok_name && pok_height && pok_weight && pok_base_experience){
        ...
        if(rows.affectedRows == 1){
            return res.status(200).json({code: 201, message: "Pokémon actualizado correctamente"});
        }
